...
def test_data_loader_uses_short_lived_cache(monkeypatch, bank):
    ...
    def fake_loader(code, requested_date, **_kwargs):
        nonlocal calls
        calls += 1
        return bank
...
def test_bank_csv_cache_round_trips_snapshot_and_market_history(monkeypatch, tmp_path, bank):
    cached = bank.model_copy(update={
        ...
        "pb_history": [.65, .66, .67], "pe_history": [5.8, 5.9, 6.0], "price_history": [5.9, 5.95, 6.0],
    })
    ...
    assert restored.pe_history == [5.8, 5.9, 6.0]
