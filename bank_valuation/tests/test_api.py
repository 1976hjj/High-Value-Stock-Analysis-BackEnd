...
def test_strategy_backtest_prepare_endpoint(monkeypatch):
    calls = []

    def fake_overview(valuation_date, refresh_cache, include_risky):
        calls.append((valuation_date, refresh_cache, include_risky))
        return {
            "module": "bank_mean_reversion_overview",
            "title": "银行低估与均值回归排序",
            "as_of_date": "2025-06-10",
    ...
    monkeypatch.setattr("bank_valuation.app.routers.bank.bank_mean_reversion_overview", fake_overview)
    response = TestClient(app).post("/api/bank/strategy-backtest/prepare", json={"valuation_date": "2025-06-10"})
    assert response.status_code == 200
    assert response.json()["module"] == "bank_mean_reversion_overview"
    assert calls == [(date(2025, 6, 10), True, True)]
